...
/**
 * Bridges Sanity's Presentation tool to a plain Astro site (no client
 * router to hook into, unlike Next.js), and — via onPerspectiveChange — sets
 * the same cookie /api/draft-mode/enable does, so switching Draft/Published
 * inside the Presentation panel itself takes effect immediately.
 */
export default function SanityVisualEditing() {
	...
	// Astro ships no client router here, so mirror plain browser navigation
	// back to the Studio.
	useEffect(() => {
		const sync = () => {
	...
	// The cookie is browser-wide, not scoped to the Presentation iframe, so
	// draft mode (and the "Exit draft mode" button) stays on in a plain tab —
	// but the click-to-edit overlay itself should only ever show inside
	// Presentation's preview pane, not on a normal visit to the site.
